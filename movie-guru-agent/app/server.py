...
async def start_user_session(
    x_user_id: Annotated[
        Union[str, None], Header(alias="x-goog-authenticated-user-email")
    ] = None,
):
    """
...
async def get_poster(poster_id: str):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error reading poster file: {e}")
...
async def get_random_movies() -> List[Dict[str, Any]]:
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error fetching random movies: {e}"
        )
    return results
